...
describe('bookings tests', function () {
    test('create a booking', function () {
        var booking = bookings_1.startBooking(destinations[0], 3);
    ...
    test('pay for a booking', function () {
        var booking = bookings_1.startBooking(destinations[0], 3);
        booking = bookings_1.processPayment(booking);
        expect(booking.paid).toBe(true);
    });
    test('complete a booking', function () {
        var booking = bookings_1.startBooking(destinations[0], 3);
        booking = bookings_1.processPayment(booking);
        booking = bookings_1.completeBooking(booking);
        expect(booking.paid).toBe(true);
...
describe('error scenarios', function () {
    test('booking must have availability', function () {
        expect.assertions(1);
        try {
            bookings_1.startBooking(destinations[6], 8);
        }
        catch (e) {
